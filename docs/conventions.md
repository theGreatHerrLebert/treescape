...
- **`highlight_clade!` alpha:** applied iff the color is opaque and `alpha != 1.0`; `round(Int, clamp(alpha * 255, 0, 255))` — clamping first so huge finite values cannot overflow, which gives the same result as Python's round-then-clamp. Julia's default `round` is ties-to-even, matching Python's. Non-finite alpha raises, as Python's `round` does.
