...
- Every wheel is checked with `twine check`, and is tested **installed**, in a clean virtual environment on its own platform, before the upload.
...
- **Order of a release:**
  1. On the release-candidate commit, run `release.yml` manually with stage `julia-libs`. It builds the libraries, creates the `jl-connector-v<version>` release, and produces `Artifacts.toml`.
  2. Commit that `Artifacts.toml`.
  3. Tag `v<version>`, which builds, tests and publishes the Python packages.

  The tagged commit's Rust code is therefore the code the artifacts were built from.
