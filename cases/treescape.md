...
- **External oracles from independent code lineages** — ete3 (CSIC), Biopython.Phylo (Biopython), and R/ggtree (Bioconductor + ggplot2) for rectangular layout; ete3 and R/ggtree for circular layout (Biopython has no circular layout). They check the Python reference, which the Rust core matches within `1e-9`, on small fixtures (2–5 tips) so far. Their agreement is strong evidence; their disagreement is documented as a convention gap and never silently absorbed by tolerance bumps.
