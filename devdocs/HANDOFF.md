...
2. **Decisions (all answered 2026-09-21):** no MATLAB is available, so the MATLAB oracle is dropped. The circular-ete3 claim is tightened to 1e-6. The course data moves to v0.7 (`devdocs/plans/v0.6-candidates.md`).
3. **Before tagging v0.6.0:** bump the versions to 0.6.0 (Cargo, pyprojects, `Project.toml`, `__version__`; the `treescape` pins in `evident.yaml` then fail `test_manifest.py` until they are updated too). Run the release tier and the `bench` job before the tag (`gh workflow run ci --ref main` runs both), update the CHANGELOG date, and ask the user before pushing the tag.
